module.exports = function (API) {
    ...
    function loginPlayer(player, data) {
        ...
        if (data.subscription) {
            if (data.subscription.tier >= 2 || calcDaysBetween(new Date(data.subscription.startDate), new Date()) < 30) {
        ...
        if (!commands) {
            ...
            try {
                ...
                commands.registerCommand(
                    ...
                    (msg, args) => {
                        if (args.length !== 2) {
                            commands.printchat("Uso: ' !register <contraseña> <repetir contraseña> '", msg.byId, "error");
                        } else {
                            if (args[0] === args[1]) {
                                        ...
                                        .then((data) => {
                                            if (data.success) {
                                                ...
                                                if (data.reason === "registered") {
                                                    ...
                                                    commands.printchat("Hubo un error, intentá más tarde.", msg.byId, "error");
                                                }
                                            }
                                        });
                                }
