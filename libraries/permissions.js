module.exports = function (API) {
    ...
    Library.call(this, "permissions", {
        // "permissions" is library's name. Every library should have a unique name.
        version: 0.1,
        author: "abc",
    ...
    this.setPlayerPermission = function (playerId, permissionId, value) {
        ...
        if (value) {
            ...
            p.add(id);
        } else ctx.playerPermissions[playerId]?.remove(id);
    };
    ...
    function showGUI() {
      ...
      `,
        ...
        gui.addScriptFromContentToWindow(
            w,
            `
    ...
    function Permissions(context) {
        ...
        this.add = function (id) {
            var idx = id >> 5,
                flag = 1 << (id & 31);
            if (thisPrm.nums[idx] == null) thisPrm.nums[idx] = flag;
            else thisPrm.nums[idx] |= flag;
        };
        this.remove = function (id) {
            var idx = id >> 5,
                flag = 1 << (id & 31);
            if (thisPrm.nums[idx] == null) return;
    ...
    function PermissionContext(uniqueId) {
        ...
        var thisCtx = this;
        this.addPermission = function (name, defaultValue = false) {
            // use this inside initialize() of your RoomConfig/Plugin.
            thisCtx.permissions.push({ name, defaultValue });
            return thisCtx.permissions.length - 1;
        };
        this.clearPermissions = function () {
            // use this inside finalize() of your RoomConfig/Plugin.
            thisCtx.permissions = [];
            thisCtx.playerPermissions = {};
        };
        this.addPlayerPermission = function (playerId, permissionId) {
            // use this to grant permission to a player.
            var p = thisCtx.playerPermissions[playerId];
            if (!p) {
                p = thisCtx.playerPermissions[playerId] = new Permissions(
                    thisCtx
                );
            }
            p.add(permissionId);
            ...
            var auth = thisLibrary.room.players.find(
                (x) => x.id == playerId
            )?.auth;
            if (!auth) return;
            ...
            if (user.permissions.findIndex((x) => x == pIdx) > -1) return;
            user.permissions.push(pIdx);
            storage.set("users", users);
        };
        this.removePlayerPermission = function (playerId, permissionId) {
            // use this to ungrant permission to a player.
        ...
        this.checkPlayerPermission = function (playerId, permissionId) {
            // use this to check permission of a player.
            if (!thisLibrary.active) return true;
            var p = thisCtx.playerPermissions[playerId];
            if (!p) {
                p = thisCtx.playerPermissions[playerId] = new Permissions(
                    thisCtx
    ...
    this.createContext = function (uniqueId) {
        // All permissions must be added once and never removed afterwards, because permissions ids must also be unique.
    ...
    this.removeContext = function (ctx) {
        ...
        if (idx >= 0) thisLibrary.allContexts.splice(idx, 1);
    };
    ...
    this.initialize = function () {
        ...
        thisLibrary.room.librariesMap.commands?.add({
            ...
            minParameterCount: 2,
            helpText:
                "Lets you change a player's permissions. Write !permissions to see all permission ids.",
            callback: ({ playerId, permissionId, value }, byId) => {
                if (!thisLibrary.room.getPlayer(playerId)) return;
                if (
                    byId != 0 &&
                    !permissionCtx.checkPlayerPermission(
                        byId,
                        permissionIds.setPermission
                    )
                ) {
                    thisLibrary.room.librariesMap.commands?.announcePermissionDenied(
                        byId
                    );
                    return;
                }
                var arr = permissionId.split(":");
                if (arr.length != 2) {
                    thisLibrary.room.librariesMap.commands?.announceError(
                        "Permission id must include exactly one ':' character. Write !permissions to see all permission ids.",
                        byId
                    );
                    return;
                }
                ...
                if (!ctx) {
                    thisLibrary.room.librariesMap.commands?.announceError(
                        "Permission context not found: " +
                            arr[0] +
                            ". Write !permissions to see all permission ids.",
                        byId
                    );
                    return;
                }
                var id = parseInt(arr[1]);
                if (value) ctx.addPlayerPermission(playerId, id);
                else ctx.removePlayerPermission(playerId, id);
            },
        });
        thisLibrary.room.librariesMap.commands?.add({
            name: "permissions",
            ...
            callback: (params, byId) => {
                ...
                thisLibrary.allContexts.forEach(({ id, permissions }) => {
                    permissions.forEach(({ name }, idx) => {
                        thisLibrary.room.librariesMap.commands?.announceInfo(
                            id + ":" + idx + " -> " + name,
                            byId
                        );
                    });
