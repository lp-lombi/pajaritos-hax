module.exports = function (API) {
    ...
    Plugin.call(this, "powerShot", true, {
        // "powerShot" is plugin's name, "true" means "activated just after initialization". Every plugin should have a unique name.
        version: "0.2",
    ...
    this.defineVariable({
        ...
        value: false,
        description:
            "If enabled, other players will be able to use the powerShot shortcut key via the powerShot_clientSide plugin.",
    });
    ...
    function powerShot(player) {
        ...
        var dx = ball.pos.x - player.disc.pos.x,
            dy = ball.pos.y - player.disc.pos.y,
    ...
    function tryPowershot(playerId) {
        var ball = that.room.gameState.physicsState.discs[0];
        var player = that.room.getPlayer(playerId),
            p = player?.disc?.pos;
        if (!p) return;
        if (
            Math.sqrt(
                (p.x - ball.pos.x) * (p.x - ball.pos.x) +
                    (p.y - ball.pos.y) * (p.y - ball.pos.y)
            ) >
            that.triggerDistance + ball.radius + player.disc.radius
        )
            return;
        powerShot(player);
    }
    ...
    this.onPlayerBallKick = function (playerId) {
        if (ticks <= that.ticksThreshold) return;
    ...
    this.onGameTick = function () {
        ...
        if (that.maxTicks > 0 && psTicks > 0) {
            psTicks++;
            if (psTicks > that.maxTicks) resetBallGravity();
        }
        ...
        if (!minDistPlayer) return;
        if (
            minSqrDist >
            that.triggerDistance + ball.radius + minDistPlayer.disc.radius
        ) {
            nearestPlayerId = null;
            ticks = 0;
            if (!swingActive)
                Utils.runAfterGameTick(() => {
                    that.room.setDiscProperties(0, {
                        color: that.room.state.stadium.discs[0].color,
                    });
                });
            return;
        }
        if (nearestPlayerId != minDistPlayer.id) ticks = 0;
    ...
    this.onCollisionDiscVsDisc = function (
        discId1,
        discPlayerId1,
        discId2,
        discPlayerId2
    ...
    this.onKeyDown = function (e) {
        if (e.code == "KeyQ") tryPowershot(0);
    };

    this.onOperationReceived = function (type, msg) {
        if (
            type == OperationType.CustomEvent &&
            msg.type == 57 &&
            that.clientEnabled
        )
    ...
    this.onVariableValueChange = function (
        addOnObj,
        variableName,
        oldValue,
        value
    ) {
        if (addOnObj != that || variableName != "ballColor") return;
        updateColor();
    };
};
